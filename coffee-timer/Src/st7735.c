...
void ST7735_DrawImageMonoRB(uint16_t x, uint16_t y, const uint8_t *data, uint16_t w, uint16_t h, uint16_t rowBytes, uint16_t color, uint16_t bgcolor) 
{
    ST7735_Select();
    ...
    for(int row = 0; row < h; row++) {
        for(int col = 0, idx = 0; col < w; col++) {
            int whichByte = col / 8 + row * rowBytes;
            int whichBit = col % 8;
            if(data[whichByte] & (1 << whichBit)) {
                lineBuffer[idx++] = color;
            } else {
                lineBuffer[idx++] = bgcolor;
            }
        }
        ST7735_WriteData((uint8_t*)lineBuffer, w*sizeof(color));
    }
...
void ST7735_FillRectangle(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color) 
{
    ...
    *ptr = color & 0xff;
    ...
    ST7735_SetMode(ST7735_DATA);
    for(y = h; y > 0; y--) {
            ST7735_TransmitData((uint8_t*)lineBuffer, sizeof(color)*w);
    }
